...
# Defino cuáles columnas voy a entregar a la red como entradas y cuáles quiero
# que aprenda a estimar como salidas.
features = [
...
# Separo los datos en tres grupos: entrenamiento para ajustar los pesos,
# validación para comparar configuraciones y prueba para la evaluación final.
train_percentage = 0.70
validation_percentage = 0.15
test_percentage = 0.15

# Estas listas forman todas las configuraciones que voy a probar manualmente.
n_list = [1, 2, 3]
units_list = [5, 10, 20]
learning_rate_list = [0.01, 0.005, 0.001]

# Mantengo estos valores iguales para que la comparación dependa solamente de
# las capas, las neuronas y la tasa de aprendizaje.
activation = "relu"
loss = "mse"
batch_size = 64
epochs = 200
...
def create_network(n, units, learning_rate):
    ...
    # Creo una red secuencial y agrego tantas capas ocultas como indique n.
    # Todas las capas ocultas usan la misma cantidad de neuronas y ReLU.
...
def request_integer(message, allowed_values):
    ...
    # Repito la solicitud para impedir que se elija una configuración que no
    # haya sido evaluada previamente.
    while True:
        try:
...
def request_float(message, allowed_values):
    ...
    # Acepto tanto punto como coma decimal y verifico el valor con np.isclose.
    while True:
        try:
...
# Primero reviso la forma y la calidad de los datos antes de entrenar la red.
...
# Uso el método IQR para identificar valores alejados de la distribución.
# Solamente los reporto porque pertenecen a condiciones válidas del simulador.
...
# Grafico cuántas muestras existen para cada valor posible de las dos salidas.
...
# Hago una división aleatoria nueva en cada ejecución porque no fijo una
# semilla. Luego conservo esta misma división para comparar justamente todas
# las configuraciones de esta ejecución.
...
# Calculo mínimos y rangos únicamente con entrenamiento para que validación y
# prueba no aporten información al preprocesamiento.
...
# Sustituyo por 1 los rangos iguales a cero para evitar divisiones entre cero.
# Las variables constantes seguirán quedando en cero después de normalizarlas.
...
for data_normalized, data_original in [
    ...
    # Aplico a los tres conjuntos la transformación min-max aprendida solamente
    # con entrenamiento, de modo que todas las variables queden comparables.
...
# Calculo la cantidad total de combinaciones antes de comenzar la búsqueda.
...
for n_grid in n_list:
    for units_grid in units_list:
        for learning_rate_grid in learning_rate_list:
            idx += 1
            # Limpio la red anterior para no acumular modelos en memoria.
            ...
            # Entreno cada combinación con los mismos conjuntos. Early Stopping
            # puede terminar antes de 200 épocas y recupera los mejores pesos.
            ...
            # Guardo resultados, tamaño del modelo y curvas para compararlos al
            # terminar la búsqueda sin utilizar todavía el conjunto de prueba.
            result = {
                "idx": idx,
                "n": n_grid,
                "units": units_grid,
...
# Ordeno las configuraciones por pérdida de validación y uso los parámetros
# como segundo criterio cuando dos resultados son parecidos.
...
# Muestro también los modelos cuyo error queda a menos de 5 % del mejor. Esto
# permite considerar una red más pequeña si mantiene un rendimiento parecido.
...
# Presento juntas las curvas MSE de todas las configuraciones para observar
# convergencia, oscilaciones y posibles diferencias entre train y validación.
...
# Repito la comparación usando MAE, que expresa el error absoluto promedio.
fig, axes = plt.subplots(grid_rows, grid_columns, figsize=(16, 23))
axes = axes.flatten()
...
# Después de revisar tablas y curvas, ingreso una de las configuraciones que sí
# fue estudiada. El programa valida los tres valores antes de continuar.
...
# Construyo una red nueva con los hiperparámetros elegidos para no reutilizar
# los pesos aprendidos durante la comparación.
tf.keras.backend.clear_session()
...
# Comparo el modelo final en entrenamiento, validación y prueba. El conjunto de
# prueba permaneció separado durante toda la elección de hiperparámetros.
...
# Devuelvo las predicciones a sus unidades originales para que los errores de
# kMc y kMt puedan interpretarse directamente.
...
# Estas curvas permiten comprobar si ambas pérdidas disminuyen juntas o si la
# validación empieza a empeorar mientras entrenamiento continúa mejorando.
...
# Comparo valores reales y estimados. La línea roja representa una predicción
# perfecta, por lo que los puntos deberían quedar cerca de ella.
...
for feature in features:
    # No puedo medir el efecto de una entrada constante porque no tiene rango.
    ...
    # Aplico ceteris paribus: cambio solamente una entrada en ±5 % y mantengo
    # todas las demás iguales para observar cuánto cambian kMc y kMt.
    ...
    # Además de la sensibilidad normalizada, calculo el cambio promedio en las
    # unidades originales de cada salida.
...
# Ordeno las entradas desde la de mayor influencia promedio hasta la menor.
