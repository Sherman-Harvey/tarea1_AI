...
# Defino las columnas que usaré como entradas y las dos variables continuas que
# la red debe aprender a estimar.
features = [
...
# Reservo una parte como prueba final y dejo el resto para entrenamiento y la
# validación cruzada interna de GridSearchCV.
train_percentage = 0.85
test_percentage = 0.15

# Esta fracción interna permite que Early Stopping observe val_loss durante
# cada ajuste. No corresponde a un tercer conjunto externo permanente.
...
# Estas listas generan las 27 configuraciones que quiero comparar.
n_list = [1, 2, 3]
units_list = [5, 10, 20]
learning_rate_list = [0.01, 0.005, 0.001]

# Dejo fijos estos valores para comparar únicamente capas, neuronas y tasa de
# aprendizaje bajo las mismas condiciones.
activation = "relu"
loss = "mse"
batch_size = 64
epochs = 200
...
def create_network(n, units, learning_rate):
    ...
    # Creo una red secuencial con n capas ocultas, la misma cantidad de neuronas
    # en cada una y activación ReLU.
...
# Reviso primero la estructura y calidad del dataset antes de entrenar.
...
# Uso IQR para detectar posibles valores alejados, pero no elimino filas porque
# representan condiciones válidas generadas por el simulador.
...
# Muestro la frecuencia de cada valor objetivo para comprobar su distribución.
...
# Realizo una división aleatoria diferente en cada ejecución porque no fijo una
# semilla. El test queda apartado y GridSearchCV trabaja solo con trainset.
...
# Calculo mínimos y rangos únicamente con entrenamiento para evitar que el test
# aporte información al preprocesamiento.
...
# Cambio a 1 los rangos iguales a cero para evitar una división entre cero. Las
# variables constantes permanecerán normalizadas en cero.
...
for data_normalized, data_original in [
    ...
    # Aplico al test exactamente la transformación min-max obtenida con train.
...
# Adapto la red de Keras a la interfaz de sklearn. Cada ajuste conserva un máximo
# de 200 épocas, aunque Early Stopping puede detenerlo antes.
...
# Indico los tres hiperparámetros que GridSearchCV debe combinar.
parameter_grid = {
    "model__n": n_list,
...
# Divido entrenamiento en cinco partes. Cada vez uso cuatro para ajustar y una
# para evaluar, repitiendo hasta que todas hayan servido como validación.
# No fijo random_state para obtener una distribución nueva en cada ejecución.
cross_validation = KFold(
    n_splits=5,
    shuffle=True
)

# Comparo mediante MSE y MAE, pero indico que el menor MSE promedio será el
# criterio para seleccionar y volver a entrenar automáticamente la mejor red.
...
# sklearn representa los errores como puntajes negativos porque siempre intenta
# maximizar. Cambio el signo para presentar MSE y MAE como errores positivos.
...
# Recupero de forma separada los tres valores de la configuración ganadora.
...
# La altura de cada barra es el MSE medio de los cinco folds y la línea vertical
# representa cuánto variaron sus resultados entre las divisiones.
...
# Tomo la mejor red, que GridSearchCV volvió a ajustar automáticamente con todo
# trainset después de terminar la comparación.
...
# Desnormalizo las predicciones para calcular errores interpretables en las
# unidades originales de kMc y kMt.
...
# Uniformo el nombre de MAE porque SciKeras puede devolverlo abreviado o con su
# nombre completo según la versión instalada. Esto no cambia ningún resultado.
...
# Comparo las curvas de entrenamiento y validación interna para revisar la
# convergencia y detectar una posible separación por sobreentrenamiento.
...
# En estas gráficas la línea roja corresponde a una predicción perfecta.
...
for feature in features:
    # Una entrada constante no tiene variación y por eso no permite calcular
    # sensibilidad.
    ...
    # Aplico ceteris paribus: modifico una entrada en ±5 % mientras conservo
    # iguales todas las demás entradas de cada muestra.
    ...
    # También convierto el efecto a las unidades originales de las salidas.
...
# Ordeno las variables desde la mayor influencia promedio hasta la menor.
